...
class CBTProvider with ChangeNotifier {
  ...
  void startRealtimeCBTUpdates(String parentId, String childId) {
    ...
    _assignedCBTListener = FirebaseFirestore.instance
        ...
        .listen((snapshot) async {
          ...
          for (var docChange in snapshot.docChanges) {
            ...
            switch (docChange.type) {
              ...
              case DocumentChangeType.removed:
                final wasRemoved = _assigned.any((a) => a.id == assigned.id);
          ...
          if (hasChanges) {
            // Merge local CBTs carefully: only add missing, do NOT overwrite Firestore updates
  ...
  Future<void> loadRemoteCBT(String parentId, String childId) async {
    ...
    _assigned = merged.values.toList();
    for (var a in _assigned) await _cbtBox?.put(a.id, a);
    await normalizeAssignedStatusesAndCleanup();
    notifyListeners();
  }
  ...
  Future<void> unassignCBT(
    ...
    try {
      ...
      // Immediately remove from local list to reflect in UI
  ...
  Future<bool> markAsCompleted(
    ...
    final assigned = _assigned.firstWhere((c) => c.id == cbtAssignedId);
  ...
  Future<void> syncPendingCompletions(String parentId) async {
    ...
    for (var id in List.from(_pendingSync)) {
      // Nullable-safe lookup
      AssignedCBT? assigned;
      try {
        assigned = _assigned.firstWhere((c) => c.id == id);
      } catch (_) {
        assigned = null;
      }
  ...
  AssignedCBT? _findAssigned(String id) {
    try {
      return _assigned.firstWhere((c) => c.id == id);
    } catch (e) {
      return null;
    }
  }
